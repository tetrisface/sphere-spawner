...
local effectEnabled = false -- Start disabled, will enable at gameframe 1
local autoEnabled = false -- Track if we've auto-enabled the effect
local intensity = 1.0
local contrast = 1.4
...
-- Auto-enable effect at gameframe 1
function widget:GameFrame(n)
	if not autoEnabled and n >= 1 then
		effectEnabled = true
		autoEnabled = true
	end
end
...
function widget:SetConfigData(data)
	if data.effectEnabled ~= nil then
		-- Don't enable effect before gameframe 1, even if saved config says so
		local gameFrame = Spring.GetGameFrame()
		if data.effectEnabled and gameFrame < 1 then
			effectEnabled = false
		else
			effectEnabled = data.effectEnabled
			if effectEnabled and gameFrame >= 1 then
				autoEnabled = true
			end
		end
	end
